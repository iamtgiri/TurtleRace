...
##### Verifications #####
userdtls = textinput("Are you new here?","If you are new user, enter : N \nIf you are old user, enter : O").lower()

df1 = pd.read_csv("usersdata.csv")

if userdtls == "n": 
    name = textinput("Username", "Enter a username")
    password = textinput("Password", "Enter a password")
    with open('usersdata.csv', 'a', newline='') as f:
        w = csv.writer(f)
        w.writerow([name, password, 5000])
    i = len(df1)-1
    score = df1["Points"][i]
    
elif userdtls == "o":
    while True:
        name = textinput("Username", "Enter your username correctly")
        password = textinput("Password", "Enter your password correctly")
        
        for i in range(0, len(df1["Name"])):
            if name == df1["Name"][i]:
                break
            
        if password == df1["Password"][i]:
            score = df1["Points"][i]
            break
penup()
goto(-200, 0)
color("yellow")
write("Successfully Login", font=("Arial", 35))
time.sleep(2)
